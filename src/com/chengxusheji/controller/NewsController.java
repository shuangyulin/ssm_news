...
public class NewsController extends BaseController {
	...
	public void list(@ModelAttribute("newsClassObj") NewsClass newsClassObj,String newsTitle,String comFrom,String addTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (newsTitle == null) newsTitle = "";
		if (comFrom == null) comFrom = "";
		if (addTime == null) addTime = "";
		if(rows != 0)newsService.setRows(rows);
		List<News> newsList = newsService.queryNews(newsClassObj, newsTitle, comFrom, addTime, page);
	    /*计算总的页数和总的记录数*/
	    newsService.queryTotalPageAndRecordNumber(newsClassObj, newsTitle, comFrom, addTime);
	    /*获取到总的页码数目*/
	...
	public String frontlist(@ModelAttribute("newsClassObj") NewsClass newsClassObj,String newsTitle,String comFrom,String addTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (newsTitle == null) newsTitle = "";
		if (comFrom == null) comFrom = "";
		if (addTime == null) addTime = "";
		List<News> newsList = newsService.queryNews(newsClassObj, newsTitle, comFrom, addTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    newsService.queryTotalPageAndRecordNumber(newsClassObj, newsTitle, comFrom, addTime);
	    /*获取到总的页码数目*/
	    int totalPage = newsService.getTotalPage();
	...
	public void OutToExcel(@ModelAttribute("newsClassObj") NewsClass newsClassObj,String newsTitle,String comFrom,String addTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(newsTitle == null) newsTitle = "";
        if(comFrom == null) comFrom = "";
        if(addTime == null) addTime = "";
        List<News> newsList = newsService.queryNews(newsClassObj,newsTitle,comFrom,addTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "News信息记录"; 
        String[] headers = { "新闻id","新闻类别","新闻标题","新闻图片","新闻来源","浏览次数","添加时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<newsList.size();i++) {
        	News news = newsList.get(i); 
        	dataset.add(new String[]{news.getNewsId() + "",news.getNewsClassObj().getNewsClassName(),news.getNewsTitle(),news.getNewsPhoto(),news.getComFrom(),news.getHitNum() + "",news.getAddTime()});
        }
        /*
